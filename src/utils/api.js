...
export async function leadsToPDF(leads, { filename = "leadflow-leads.pdf", title = "Leads Export" } = {}) {
  const [{ jsPDF }, { default: autoTable }] = await Promise.all([
    import("jspdf"),
    import("jspdf-autotable"),
  ]);
  const doc = new jsPDF({ orientation: "landscape", unit: "mm", format: "a4" });
  const accent = [240, 132, 93];
  const dark = [36, 33, 44];
  ...
  doc.text(`${new Date().toLocaleString()}  |  ${leads.length} lead(s)`, 14, 20);

  autoTable(doc, {
    head: [["ID", "Lead Name", "Business", "Email", "Phone", "Category", "Status", "Source", "City", "State", "Country", "Date"]],
    body: leads.map(l => [
      l.id || "",
      l.leadName || "",
      l.businessName || "",
      l.email || "",
      l.phone || "",
      (l.category === "Other" && l.customCategory) ? l.customCategory : (l.category || ""),
      l.leadStatus || "",
      l.leadSource || "",
      l.city || "",
      l.state || "",
    ...
    startY: 32,
    styles: { font: "helvetica", fontSize: 7.5, cellPadding: 2, textColor: [50, 47, 60], lineColor: [221, 220, 230], lineWidth: 0.15 },
    headStyles: { fillColor: accent, textColor: 255, fontStyle: "bold", fontSize: 7.5 },
    alternateRowStyles: { fillColor: [246, 244, 250] },
    margin: { left: 10, right: 10 },
    columnStyles: { 0: { cellWidth: 20 }, 3: { cellWidth: 42 }, 4: { cellWidth: 30 }, 5: { cellWidth: 24 }, 6: { cellWidth: 20 } },
  });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Generated by LeadFlow CRM  |  Page ${i} of ${pageCount}`, 14, doc.internal.pageSize.getHeight() - 7);
  }
